...
// ───── Pexels 이미지 검색 (주제에 맞는 사진 1장 URL 반환) ─────
...
function getBaseLayout(json = {}) {
    ...
    const ogImage = json?.hero?.heroImage || 'https://picsum.photos/seed/og/1200/630';
...
Validatix 구현 범위 (가장 중요한 절대 규칙)입니다.
Validatix가 실제로 만들 수 있는 기능은 다음뿐입니다.
- 단일 데이터 테이블의 입력, 수정, 삭제, 조회
- 기준값 미달 항목을 화면 목록으로 표시 (예: 최소 수량보다 적은 항목 표시)
- 전체 데이터 기준 통계 (개수, 합계, 평균)와 막대 차트
- 단일 계정 로그인 및 회원가입
- 구독 결제, 이미지 업로드
다음 기능은 만들 수 없습니다. 역질문에서 먼저 제안하지 말고, FINAL_PRD의 핵심 기능이나 UI 구조에도 절대 넣지 마세요.
- 발주, 주문, 외부 시스템 연동
- 푸시, 문자, 카카오, 이메일 등 능동 알림 통지
- 입출고 기록 기반 자동 수량 계산
- 직원 계정, 권한 분리, 다중 사용자
- 엑셀 가져오기 및 내보내기, 외부 API 연동, AI 분석
사용자가 위 불가 기능을 직접 원하더라도, 구현되지 않는다고 솔직히 알리고 구현 가능한 범위로 대안을 제시하세요. 불가 기능에 대해 "재생성하면 추가된다"고 말하지 마세요.
...
- 구현 가능 범위 안의 변경(필드 추가, 디자인 변경, 텍스트 수정 등)을 언급할 때만 "Validatix에서 언제든 재생성하면 됩니다"를 포함하세요. 위 구현 불가 기능에는 이 표현을 절대 쓰지 말고, 만들 수 없다고 솔직히 알리세요.
...
- ALL pricing plans MUST use the SAME currency symbol. If any plan's price uses the currency from the idea/PRD (e.g. ₩), EVERY plan including Free MUST use that same currency (e.g. "₩0", never "$0"). NEVER mix $ and ₩ across plans.
- pricing features_ko/features_en MUST list ONLY capabilities the system actually builds: single-table add/edit/delete, list/table view, threshold display, stat cards, a bar chart, single-account login. NEVER list capabilities the system does NOT build — no detail/sub-pages, no file/photo/image upload, no notifications/alerts, no sorting/filtering, no automatic calculation, no multi-user/sharing. If the PRD mentions any of these, exclude them from pricing features.
...
export default function HeroSection() {
  ...
  return (
    <section id="hero" className="vx-hero">
      <img className="vx-hero-bg" src="${d.heroImage || 'https://picsum.photos/seed/citystreet999/1200/600'}" alt="hero" />
...
async function buildComponents(json, sendLog, isPaidUser = false, lang = 'ko') {
    ...
    // ───── Pexels로 기능 카드 이미지(customImage) 채우기 (Tier 1: 이미지 주제 일치) ─────
    // 각 기능의 image_seed(주제 명사)로 Pexels를 검색해 customImage에 주소를 채운다.
    // 이미 customImage가 있으면(사용자 업로드/AI 이미지) 건너뛰어 보존한다.
    // 키가 없거나 실패하면 customImage를 비워두고 buildFeatures가 기존 picsum 폴백을 쓴다.
    if (process.env.PEXELS_API_KEY && json.features && Array.isArray(json.features.items) && json.features.items.length > 0) {
        sendLog(isKo ? `[Validatix] 🖼️ Pexels로 기능 이미지 매칭 중...` : `[Validatix] 🖼️ Matching feature images via Pexels...`);
        await Promise.all(json.features.items.map(async (item) => {
            if (!item || item.customImage) return;
            const keyword = item.image_seed || (item.title && (item.title.en || item.title.ko)) || '';
            if (!keyword) return;
            const url = await fetchPexelsImage(keyword);
            if (url) item.customImage = url;
        }));
        sendLog(isKo ? `[Validatix] ✅ Pexels 이미지 매칭 완료.` : `[Validatix] ✅ Pexels image matching done.`);
    }
    
    // ───── Pexels로 Hero 배경 이미지(heroImage) 채우기 (2단계) ─────
    // 앱 주제를 대표하는 첫 기능의 image_seed로 검색해 hero.heroImage에 채운다.
    // 이미 heroImage가 있으면 보존하고, 키가 없거나 실패하면 picsum 폴백이 뜬다.
    if (process.env.PEXELS_API_KEY && json.hero && !json.hero.heroImage) {
        const heroKeyword =
            (json.features && Array.isArray(json.features.items) && json.features.items[0] && json.features.items[0].image_seed) ||
            (json.hero.title && (json.hero.title.en || json.hero.title.ko)) ||
            (json.navbar && json.navbar.logo) || '';
        if (heroKeyword) {
            const heroUrl = await fetchPexelsImage(heroKeyword);
            if (heroUrl) json.hero.heroImage = heroUrl;
        }
    }
    ...
    const resolveComponent = (name) => {
        if (aiComponents[name] && !FORCE_FALLBACK.has(name)) {
            ...
            // AI가 만든 Hero 코드의 첫 picsum을 heroImage로 교체 (Hero 배경은 보통 1장)
            if (name === 'HeroSection' && json.hero?.heroImage) {
                let heroReplaced = false;
                code = code.replace(/https:\/\/picsum\.photos\/seed\/[^"'\s]+/g, (match) => {
                    if (!heroReplaced) { heroReplaced = true; return json.hero.heroImage; }
                    return match;
                });
            }
            return code;
...
const nextConfig = {
  ...
  images: { remotePatterns: [{ protocol: 'https', hostname: 'picsum.photos' }, { protocol: 'https', hostname: 'images.pexels.com' }] },
  async headers() {
